#!/bin/bash
# EVALUATION (FIGER): Zero-shot performance of roberta-large with fac+lin adapters (CONCAT fusion).
echo "====== EVALUATING (FIGER): ZERO-SHOT FAC+LIN (CONCAT) ======"
python examples/run_finetune_figer_adapter.py \
    ...
    --comment 'eval_figer_zeroshot_fac_lin_concat' \
    --max_seq_length=256  \
